...
// // Helper to get dates within 7-day range
// const getDatesWithinNext7Days = (startDate, workingDates) => {
//   if (!startDate || !workingDates.length) return [];
...
// // Helper to calculate next occurrence dates based on frequency within 7-day range
// const getNextOccurrences = (task, workingDates, currentDate) => {
...
//   if (!taskStartDate) return occurrences;
...
//   // For recurring tasks, calculate occurrences within 7 days
//   let iterationCount = 0;
//   const maxIterations = 365; // Safety limit

//   while (nextDate <= endDate && iterationCount < maxIterations) {
//     if (nextDate >= today) {
...
//   // --- Main build: Create combined events for next 7 days ---
//   const generateCombinedDateMap = (uniqueTasks, workingDates) => {
//     let map = {};
//     const today = new Date();
//     today.setHours(0, 0, 0, 0);

//     // Filter working dates to next 7 days
//     const next7WorkingDates = getDatesWithinNext7Days(today, workingDates);
...
//       // Get all occurrences within next 7 days
//       const occurrences = getNextOccurrences(task, next7WorkingDates, today);
...
//       // Step 4: Build combined per-date map for next 7 days
...
//                     Task Calendar (Next 7 Days)
//                   </h1>
//                   <p className="text-xs sm:text-sm text-gray-600 mt-1 font-medium">
//                     Unique Sheet • Next 7 Days View • Role:{" "}
...
//                     <span className="text-green-600 font-semibold">✓ {stats.completed} Completed</span>
//                     <span className="text-blue-600 font-semibold">🔄 {stats.pending} Pending</span>
//                     <span className="text-gray-700 font-semibold">📋 {stats.total} Total</span>
//                   </div>
//                 </div>
//               </div>
...
//                               Frequency: <span className="font-semibold">{t.freq || "One-Time"}</span> •
//                               Next Occurrence: <span className="font-semibold">{formatDate(t.occurrenceDate || t.startDate)}</span>
//                             </span>
//                           </div>
//                           {t.remarks && (
