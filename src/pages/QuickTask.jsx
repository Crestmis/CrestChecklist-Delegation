...
export default function QuickTask() {
  ...
  const CONFIG = {
    SHEET_ID: "1hP6T2p2raJaxNSG3LtFbnIexNxJmU2TskmKDwaBX2hE",
      ...
      if (data?.table?.rows) {
        ...
        data.table.rows.slice(1).forEach((row) => {
          if (row.c) {
            ...
            if (
              doerName.toLowerCase().trim() ===
              loggedInUsername.toLowerCase().trim()
            ) {
              foundUser = {
                name: doerName,
        ...
        if (foundUser) {
          ...
          throw new Error(
            `User "${loggedInUsername}" not found in Whatsapp sheet. Please contact administrator.`
          );
        }
      } else {
    ...
    try {
      ...
      if (data?.table?.rows) {
        ...
        const transformedData = rows
          .map((row, rowIndex) => {
            const baseData = {
              _id: `checklist_${rowIndex}_${Math.random()
                .toString(36)
                .substring(2, 15)}`,
        ...
        transformedData.forEach((task) => {
          const key = `${task.Name?.toLowerCase().trim()}_${task[
            "Task Description"
          ]
            ?.toLowerCase()
            .trim()}`;
    ...
    try {
      ...
      if (data?.table?.rows) {
        ...
        const transformedData = rows.map((row, rowIndex) => {
          const baseData = {
            _id: `delegation_${rowIndex}_${Math.random()
              .toString(36)
              .substring(2, 15)}`,
            _rowIndex: rowIndex + 2,
        ...
        if (userRole === "admin" || userRole === "main admin") {
          ...
          filteredData = transformedData.filter((item) => {
            const itemName = (item.Name || "").toString().toLowerCase().trim();
            const itemGivenBy = (item["Given By"] || "")
              .toString()
              .toLowerCase()
              .trim();
            const currentUserLower = currentUser.toLowerCase().trim();
  ...
  const getFilterOptions = () => {
    ...
    const names = [...new Set(currentTasks.map((task) => task.Name))].filter(
    ...
    const frequencies =
      activeTab === "checklist"
        ? [...new Set(currentTasks.map((task) => task.Frequency))].filter(
            (freq) => freq && typeof freq === "string" && freq.trim() !== ""
          )
        : [...new Set(currentTasks.map((task) => task.Freq))].filter(
            (freq) => freq && typeof freq === "string" && freq.trim() !== ""
          );
  ...
  const { names: currentNames, frequencies: currentFrequencies } =
    getFilterOptions();
  ...
  const filteredChecklistTasks = tasks
    .filter((task) => {
  ...
  // console.log("delegationTasks",delegationTasks);

  const department = sessionStorage.getItem("department");

  const fillteredChecklistByMainAdmin =
    sessionStorage.getItem("role") === "main admin"
      ? filteredChecklistTasks
      : filteredChecklistTasks.filter((item) => item.Department === department);

  const filteredDelegationByMainAdming =
    userRole === "main admin"
      ? delegationTasks
      : delegationTasks.filter((item) => item.Department === department);
        ...
        <>
          {activeTab === "checklist" ? (
              ...
              >
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    ) : fillteredChecklistByMainAdmin.length > 0 ? (
                      fillteredChecklistByMainAdmin.map((task) => (
            ...
            <DelegationPage
              ...
              userRole={userRole}
              CONFIG={CONFIG}
              delegationTasks={filteredDelegationByMainAdming}
