      ...
      <div className="bg-white rounded-lg p-6 w-96 max-w-md">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div className="text-sm text-purple-600 mt-1">
              Only 'One-Time' frequency. Tasks will be stored in the Delegation
              sheet.
            </div>
          </button>
        </div>
...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1hP6T2p2raJaxNSG3LtFbnIexNxJmU2TskmKDwaBX2hE";
  ...
  useEffect(() => {
    const fetchDoerOptions = async () => {
      try {
        ...
        if ((userRole !== "admin" && userRole !== "main admin") && selectedDoer) {
          setFormData((prev) => ({
            ...prev,
            doer: selectedDoer,
          }));
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1hP6T2p2raJaxNSG3LtFbnIexNxJmU2TskmKDwaBX2hE/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1hP6T2p2raJaxNSG3LtFbnIexNxJmU2TskmKDwaBX2hE";
  ...
  const generateTasks = async () => {
    ...
    // For one-time tasks (delegation) OR checklist tasks
    if (formData.frequency === "one-time" || selectedTaskType === "checklist") {
      // Use the same single-task generation logic for both delegation AND checklist
      const selectedDate = new Date(date);
    ...
    try {
      ...
      if (formData.frequency !== "one-time") {
        ...
        const tasksDataUnique = tasksToSubmit.map((task, index) => ({
          ...
          enableReminders: task.enableReminders ? "Yes" : "No",
          requireAttachment: task.requireAttachment ? "Yes" : "No",
          lastTimeUpdateDate: task.dueDate,  // Same as Task Deadline Date (startDate)
        }));
      ...
      setFormData({
        department: "",
        givenBy: "",
        doer: (userRole !== "admin" && userRole !== "main admin") && username ? formData.doer : "",
        description: "",
        frequency: "daily",
        enableReminders: true,
        requireAttachment: false,
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm text-purple-600">
                  All frequencies of Daily,Weekly,Monthly,Yearly etc. Tasks will
                  be stored in the Checklist sheet.
                </p>
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm text-purple-600">
                  Only 'One-Time' frequency. Tasks will be stored in the
                  Delegation sheet.
                </p>
                <button className="mt-4 bg-purple-600 text-white px-4 py-2 rounded-md hover:bg-purple-700 transition-colors">
                  Select Delegation
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const resetFormData = {
                        ...
                        givenBy: "",
                        doer:
                          (userRole !== "admin" && userRole !== "main admin") && username ? formData.doer : "",
                        description: "",
                        frequency: "daily",
                        enableReminders: true,
                        requireAttachment: false,
