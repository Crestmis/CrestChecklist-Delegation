...
export default function DelegationPage({
  ...
  const filteredDelegationByMainAdming =
    userRole === "main admin"
      ? filteredTasks
      : filteredTasks.filter((item) => item.Department === department);
    ...
    <div className="mt-4 rounded-lg border border-purple-200 shadow-md bg-white overflow-hidden">
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-100 p-4">
        ...
        <p className="text-purple-600 text-sm">
          {userRole === "admin"
            ? `Showing ${filteredDelegationByMainAdming.length} delegation tasks from all users`
            : `Showing ${filteredDelegationByMainAdming.length} delegation tasks for ${currentUser}`}
        </p>
      </div>
      ...
      >
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            ) : filteredDelegationByMainAdming.length > 0 ? (
              filteredDelegationByMainAdming.map((task) => (
