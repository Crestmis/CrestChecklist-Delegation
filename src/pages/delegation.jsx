...
function DelegationDataPage() {
  ...
  console.log(accountData, "account dateA");
  ...
  console.log("filteredAccountData count:", filteredAccountData.length);
  console.log("filteredAccountData sample:", filteredAccountData[0]);
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      if (data.table && data.table.rows) {
        ...
        console.log("Using data.table.rows, count:", rows.length);
      } else if (Array.isArray(data)) {
        rows = data;
        console.log("Using array data, count:", rows.length);
      } else if (data.values) {
        rows = data.values.map((row) => ({
          c: row.map((val) => ({ v: val })),
        }));
        console.log("Using data.values, count:", rows.length);
      }
      ...
      rows.forEach((row, rowIndex) => {
        ...
        console.log(
          `Filtering check - userRole: "${userRole}", username: "${username}", adminDepartment: "${adminDepartment}"`
        );
        ...
        if (userRole !== "admin") {
          ...
          console.log(
            `User filter: taskAssignedTo="${taskAssignedTo}", username="${username}"`
          );
          if (
            !taskAssignedTo ||
            ...
            console.log("Task filtered out - not assigned to user");
            return;
          }
        } else if (userRole === "admin") {
          // ✅ Admin: Show ALL tasks by default
      ...
      console.log(
        `After filtering - Loaded ${allDelegationData.length} tasks for user: ${username}, role: ${userRole}, dept: ${adminDepartment}`
      );
      console.log("Sample task data:", allDelegationData[0]);
  ...
  useEffect(() => {
    ...
    if (userRole && username) {
      console.log(
        `Starting fetch: userRole=${userRole}, username=${username}, adminDepartment=${adminDepartment}`
      );
      fetchSheetData();
    }
  }, [
  ...
  const handleSubmit = async () => {
    ...
    try {
          ...
          batch.map(async (id) => {
            ...
            if (shouldCreateNewTask) {
              ...
              if (!newTaskResult.success) {
                throw new Error(
                  `Failed to create new task: ${newTaskResult.error}`
                );
              }
              ...
              if (!historyResponse.ok) {
                throw new Error(
                  "Failed to submit history record to DELEGATION DONE sheet"
                );
              }
              ...
              console.log(
                "Successfully submitted extension to DELEGATION DONE sheet"
              );
            } else {
              ...
              console.log(JSON.stringify(insertFormData), "insertFormData");
              ...
              if (!response.ok) {
                throw new Error(
                  "Failed to submit task record to DELEGATION DONE sheet"
                );
              }
              ...
              console.log(
                "Successfully submitted regular task to DELEGATION DONE sheet"
              );
            }
          })
        );
      ...
      setSuccessMessage(
        `Successfully processed ${selectedItemsArray.length} task record(s)!`
      );
      setSelectedItems(new Set());
      setAdditionalData({});
      setRemarksData({});
      ...
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex space-x-4">
            ...
            {!showHistory && (
              <button
                onClick={userRole !== "admin" && handleSubmit}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredAccountData.length > 0 ? (
                    filteredAccountData.map((account) => {
                      console.log(account, "filteraccount");
